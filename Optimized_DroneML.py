...
process_frame_interval = 4
frame_count = 0

# Función para mover el dron basado en la posición del centroide
def move_drone_based_on_centroid(centroid_x, width):
    if centroid_x > width // 2 + 30:
        print("Moviendo a la derecha")
        bebop.fly_direct(roll=2, pitch=0, yaw=0, vertical_movement=0, duration=1)
    ...
    # Si está centrado horizontalmente, avanzar hacia el objeto
    if abs(centroid_x - width // 2) < 30:
    ...
    if img is not None:
        ...
        if frame_count % process_frame_interval == 0:
            ...
            for result in results[0].boxes.xyxy:  # x1, y1, x2, y2
                ...
                cv2.circle(annotated_img, (center_x, (y1 + y2) // 2), 5, (0, 255, 0), -1)  # Círculo verde

                # Mover el dron basado en la posición del centroide
                move_drone_based_on_centroid(center_x, 640)
